...
import '../styling/ImageCaptioningStyle.css'; 
  ...
  const readURL = async (input) => {
    if (input.files && input.files[0]) {
      ...
      reader.onload = async function (e) {
        ...
        try {
          const response = await axios.post('http://localhost:8004/predict', formData, {
            headers: {
    ...
    <div className="file-upload">
      ...
      <div className="image-upload-wrap" onDragOver={handleDragOver} onDragLeave={handleDragLeave}>
        ...
        <div className="drag-text">
          <h3>Drag and drop a file or select add Image to caption it</h3>
        </div>
      </div>
